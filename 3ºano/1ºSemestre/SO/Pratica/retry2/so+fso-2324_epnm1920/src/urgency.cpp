...
void init_simulation(int np)
{
   ...
   for(int i=0; i< hd->num_patients; i++){
      mutex_init(&hd->all_patients[i].mutexo,NULL);
      cond_init(&hd->all_patients[i].isTreated,NULL);
...
void term_simulation(int np) {
   ...
   for(int i=0; i< hd->num_patients; i++){
      mutex_destroy(&hd->all_patients[i].mutexo);
      cond_destroy(&hd->all_patients[i].isTreated);
   }
   free(hd);
   hd = NULL;
}
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   mutex_lock(&hd->all_patients[patient].mutexo);
   hd->all_patients[patient].done = 1;

   cond_broadcast(&hd->all_patients[patient].isTreated);
   mutex_unlock(&hd->all_patients[patient].mutexo);
   return 0;
}
...
void patient_wait_end_of_consultation(int id)
{
   ...
   mutex_lock(&hd->all_patients[id].mutexo);
   
   while(hd->all_patients[id].done == 0){
      cond_wait(&hd->all_patients[id].isTreated,&hd->all_patients[id].mutexo);
   }

   mutex_unlock(&hd->all_patients[id].mutexo);
   check_valid_name(hd->all_patients[id].name);
...
void *nurse_life(void *id){
   int* p =(int*) id;
   while(1){
      int termination = nurse_iteration(*p);
      if(termination ==1 ){
         break;
      }
   }
