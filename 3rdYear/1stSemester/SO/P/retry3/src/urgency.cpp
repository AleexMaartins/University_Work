...
void init_simulation(int np)
{
   ...
   for(int i = 0; i < MAX_PATIENTS; i++) {
      ...
      mutex_init(&hd->all_patients[i].mutexy, NULL); // inicializar mutex
...
void term_simulation(int np) {
   ...
   for(int i = 0; i < MAX_PATIENTS; i++) {
      cond_destroy(&hd->all_patients[i].isTreated); //inicializar var de cond
      mutex_destroy(&hd->all_patients[i].mutexy); // inicializar mutex
   }
   free(hd);
   hd = NULL;
}
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   mutex_lock(&hd->all_patients->mutexy);
   hd->all_patients[patient].done = 1;
   
   cond_broadcast(&hd->all_patients->isTreated);
   mutex_unlock(&hd->all_patients->mutexy);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   mutex_lock(&hd->all_patients[id].mutexy);
   while(&hd->all_patients[id].done == 0){
      cond_wait(&hd->all_patients[id].isTreated,&hd->all_patients[id].mutexy);      
   }

   printf("\e[30;01mPatient %s (number %d): health problems treated\e[0m\n", hd->all_patients[id].name, id);
   mutex_unlock(&hd->all_patients[id].mutexy);
}
...
void *doctor_life(void *id){
   ...
   int id2 = *p;
   while(1){
      int termination = doctor_iteration(id2);
      if(termination ==1){ ///////////
         break;
      }
   }
...
void *nurses_life( void *id){
   int* p = (int*)id;
   int id2 = *p;
   while(1){
      int termination = nurse_iteration(id2);
      if(termination==1){
      break;
      }
   }
...
int main(int argc, char *argv[])
{
   ...
   pthread_t doctor[ndoctors];
   int args1[ndoctors];
   for(int i=0; i<ndoctors;i++){
      args1[i]=i;
      thread_create(&doctor[i],NULL,doctor_life,&args1[i]);
   }

   pthread_t nurse[nnurses];
   int args2[nnurses];
   for(int i=0; i<nnurses;i++){
      args2[i]=i;
      thread_create(&nurse[i],NULL,nurses_life,&args2[i]);
   }

   pthread_t patient[npatients];
   int args3[npatients];
   for(int i=0; i<npatients;i++){
      args3[i]=i;
      thread_create(&patient[i],NULL,patientlife,&args3[i]);
   }


   for(int i=0; i<npatients;i++){
      thread_join(patient[i],NULL);
   }
   ...
   for(int i=0; i<ndoctors;i++){
      thread_join(doctor[i],NULL);
   }
   for(int i=0; i<nnurses;i++){
      thread_join(nurse[i],NULL);
   }
